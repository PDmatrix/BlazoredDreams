...
namespace BlazoredDreams.Persistence.Repositories
{
	public class UserLikesRepository : BaseRepository, IUserLikesRepository
	{
		...
		public Task AddLikeAsync(int userId, int postId, CancellationToken ct = default)
        {
            const string sql = 
				@"INSERT INTO user_likes (user_id, post_id) VALUES (@userId, @postId)";
            return Connection.ExecuteAsync(sql, new {userId, postId}, Transaction);
        }

		public Task RemoveLikeAsync(int userId, int postId, CancellationToken ct = default)
        {
            const string sql = 
